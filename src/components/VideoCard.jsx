...
function VideoCard({ video }) {
  ...
  useEffect(() => {
    const fetchThumbnail = async () => {
      try {
        const response = await axios.get(`${BACKEND_API_URL}/videos/${video.linkid}/thumbnail`);
  ...
  const defaultThumbnail = 'https://via.placeholder.com/320x180?text=No+Thumbnail'; // Placeholder image

  return (
    <div className="card h-100 bg-dark text-white border-secondary shadow-sm">
      <Link to={`/video/${video.linkid}`}>
        {loadingThumbnail ? (
          <div className="d-flex justify-content-center align-items-center bg-secondary" style={{ height: '180px', width: '100%' }}>
            <div className="spinner-border text-light" role="status">
              <span className="visually-hidden">Loading thumbnail...</span>
            </div>
          </div>
        ) : errorThumbnail || !thumbnailUrl ? (
          <img src={defaultThumbnail} className="card-img-top" alt="Thumbnail not available" />
        ) : (
      ...
      <div className="card-body">
        ...
        <Link to={`/video/${video.linkid}`} className="btn btn-primary btn-sm stretched-link">
          Watch Now
        </Link>
      </div>
    </div>
