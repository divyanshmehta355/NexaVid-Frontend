...
function UploadVideo() {
    ...
    try {
      ...
      setLocalUploadError(
        err.response?.data?.message ||
          err.message ||
          "An unexpected error occurred during local upload."
      );
    } finally {
      setUploading(false);
    }
  ...
  const handleRemoteUpload = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/remote-upload`,
        payload,
        {
      ...
      if (response.data.success) {
        ...
        setRemoteUploadError(
          response.data.message || "Remote upload initiation failed."
        );
      }
    } catch (error) {
      ...
      if (axios.isAxiosError(error) && error.response) {
        setRemoteUploadError(
          error.response.data.message ||
            "Server error initiating remote upload."
        );
      } else {
  ...
  const checkRemoteUploadStatus = useCallback(
    async (id) => {
      if (!id) return;

      try {
        const response = await axios.get(
          `${API_BASE_URL}/api/remote-upload-status/${id}`
        );
        ...
        if (response.data.success) {
          ...
          const totalMB = bytesTotal
            ? (bytesTotal / (1024 * 1024)).toFixed(2)
            : "?";

          setRemoteUploadStatus(
            `Status: ${status} (${loadedMB} MB / ${totalMB} MB)`
          );
          ...
          setRemoteUploadStatus(
            `Error checking status: ${response.data.message}`
          );
          if (statusIntervalId) {
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Upload your video content to Seductive Streams."
        />
      </Helmet>

      <div className="card bg-dark text-white border-secondary shadow-lg p-4 mb-5">
        <h2 className="card-title text-center mb-4 text-info">
          Upload New Video
        </h2>
        <div className="card-body">
          ...
          <div className="mb-5 p-4 border rounded border-secondary bg-dark-subtle">
            ...
            <p className="text-muted">
              Upload a video directly from your device. For large files, this
              might take some time.
            </p>
            <div className="mb-3">
              <label htmlFor="videoFileInput" className="form-label text-white">
                Select Video File
              </label>
              <input
            ...
            {localUploadedVideoData && (
              <div className="alert alert-success mt-3" role="alert">
                ...
                <p>
                  <strong>Filename:</strong> {localUploadedVideoData.fileName}
                </p>
                <p>
                  <strong>Stream URL:</strong>{" "}
                  <a
                    href={localUploadedVideoData.streamUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="alert-link"
                  >
                    {localUploadedVideoData.streamUrl}
                  </a>
                </p>
                <p>
                  <strong>File ID:</strong> {localUploadedVideoData.fileId}
                </p>
                <p>
                  You can find this video on the{" "}
                  <a href="/" className="alert-link">
                    Home page
                  </a>{" "}
                  after Streamtape finishes processing.
                </p>
              </div>
            )}
          ...
          <div className="p-4 border rounded border-secondary bg-dark-subtle">
            ...
            <p className="text-muted">
              Provide a publicly accessible URL for Streamtape to directly
              download the video. This is ideal for very large files as it
              bypasses your server.
            </p>
            <div className="mb-3">
              <label htmlFor="remoteUrl" className="form-label text-white">
                Video URL
              </label>
              <input
                type="text"
            ...
            <div className="mb-3">
              <label htmlFor="remoteFileName" className="form-label text-white">
                Optional Custom Name (e.g., My Awesome Video.mp4)
              </label>
              <input
                type="text"
                id="remoteFileName"
            ...
            <div className="d-grid gap-2 mt-4">
              ...
              >
                {isRemoteUploading ? (
                  ...
                  <>
                    <i className="bi bi-link-45deg me-2"></i> Initiate Remote
                    Upload
                  </>
                )}
              </button>
            ...
            {remoteUploadId && (
              <div className="alert alert-info mt-3" role="alert">
                ...
                <p className="mb-1">
                  <strong>ID:</strong> <code>{remoteUploadId}</code>
                </p>
                <p className="mb-1">
                  <strong>Status:</strong> {remoteUploadStatus}
                </p>
                {remoteStreamtapeFinalUrl && (
                  <p className="mb-1">
                    <strong>Final URL:</strong>{" "}
                    <a
                      href={remoteStreamtapeFinalUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="alert-link"
                    >
                ...
                <small className="text-muted d-block mt-2">
                  {remoteUploadStatus.includes("finished") ||
                  remoteUploadStatus.includes("failed")
